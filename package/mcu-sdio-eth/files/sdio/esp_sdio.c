...
/* ATBM6441 HIF keep-awake: the WSM control driver holds bit12 of the HIF
 * CONTROL register (reg id 0x0001 -> SDIO addr 4, 16-bit LE); its rmmod
 * path (FW_SLEEP) clears it, after which the battery-oriented MCU firmware
 * naps and posts no TX credits. Set the bit again so the data path runs. */
#define ATBM_HIF_CONTROL_ADDR	4
#define ATBM_HIF_CONT_WUP_BIT	(1 << 12)

static void esp_legacy_wake_slave(struct esp_sdio_context *context)
{
	u32 tmp __aligned(4) = 0;
	u16 val;
	int ret;
	...
	ret = sdio_memcpy_fromio(context->func, &tmp, ATBM_HIF_CONTROL_ADDR, 2);
	val = tmp & 0xffff;
	if (!ret && !(val & ATBM_HIF_CONT_WUP_BIT)) {
		tmp = val | ATBM_HIF_CONT_WUP_BIT;
		ret = sdio_memcpy_toio(context->func, ATBM_HIF_CONTROL_ADDR, &tmp, 2);
		esp_info("legacy slave wake: ctrl 0x%04x -> 0x%04x ret=%d\n",
			 val, (u16)tmp, ret);
	}
	sdio_release_host(context->func);
}

/* Legacy slaves (ATBM6441 MCU, pre-2022 esp-hosted fw) never send the
 * ESP_PRIV_EVENT_INIT bootup event -- they expect the host to bring the
 * interfaces up unconditionally (as the original vendor driver did).
 * If no INIT event has arrived shortly after probe, force the bring-up
 * with minimal capabilities (WLAN over SDIO, no checksum offload). */
static void esp_legacy_init_work_fn(struct work_struct *work)
{
	...
	esp_warn("no INIT event from slave; assuming legacy slave, forcing bring-up\n");
	esp_legacy_wake_slave(&sdio_context);
	generate_slave_intr(&sdio_context, BIT(ESP_OPEN_DATA_PATH));
	adapter->capabilities = ESP_WLAN_SDIO_SUPPORT;
	atomic_set(&adapter->state, ESP_CONTEXT_READY);
	if (esp_add_card(adapter)) {
		esp_err("legacy bring-up: network interface init failed\n");
		atomic_set(&adapter->state, ESP_CONTEXT_DISABLED);
	}
}
static DECLARE_DELAYED_WORK(esp_legacy_init_work, esp_legacy_init_work_fn);
...
static const struct sdio_device_id esp_devices[] = {
	/* ATBM6441 MCU runs an ESP-Hosted-style slave; match any SDIO device
	 * (the only SDIO function on this board is the WiFi/MCU combo).
	 * The ATBM control driver must be rmmod'ed first to free the function. */
...
static void esp_handle_isr(struct sdio_func *func)
{
	...
	if (!ret && (regs[0] & ESP_SLAVE_RX_NEW_PACKET_INT)) {
		...
		context->prefetch_len_valid = true;
	}
...
static void esp_remove(struct sdio_func *func)
{
	...
	cancel_delayed_work_sync(&esp_legacy_init_work);
	...
	while (!kthread_should_stop()) {
		...
		buf_needed = (aggr_len + tx_aggr_size - 1) / tx_aggr_size;
			...
			if (!ret) {
				...
				/* ATBM slave may have napped (keep-awake bit cleared);
				 * poke it so the next attempt finds credits. */
				esp_legacy_wake_slave(&sdio_context);
				continue;
			}
				  ...
				  const struct sdio_device_id *id)
{
	...
	schedule_delayed_work(&esp_legacy_init_work, msecs_to_jiffies(3000));
